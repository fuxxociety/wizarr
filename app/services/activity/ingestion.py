...
class ActivityIngestionService:
    ...
    def _commit(self) -> bool:
        ...
        PostgreSQL handles concurrent writes via MVCC, so retries are generally
        not needed. Only critical serialization failures should be retried at
        ...
        try:
            ...
            self.logger.error("Database commit failed: %s", exc, exc_info=True)
            db.session.rollback()
            return False
